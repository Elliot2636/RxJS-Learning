...
内含三种函数 next error complete【备注：error和complete只会有一个生效】
...
Obsever只能单独订阅一个属于自己Observables，也就是说Observables与Obsever是一对一的关系，但是Subject是一种特殊的Observables可被多个Obsever订阅
...
Subject自身也可以放在Subcribe中作为订阅别的Observable
...
BehaviorSubject 每当有新的Observer订阅，新订阅的Observer就会立刻收到当前值(普通Subject只有Observable变化时[此处的变化是指出现新的Observable]会激活Observer)
...
A`ReplaySubject`与 a 的相似之处`BehaviorSubject`在于它可以将旧值发送给新订阅者，但它也可以*记录*Observable 执行的一部分。*A`ReplaySubject`记录来自 Observable 执行的多个值，并将它们重播给新订阅者。*创建 时`ReplaySubject`，您可以指定要重放的值数量：
...
- [of](https://rxjs.dev/api/index/function/of)
    - [创建一个Observable按顺序返回传入参数]
    - `of<T>(...args: (SchedulerLike | T)[]): Observable<T>`
- [from](https://rxjs.dev/api/index/function/from)
    - [创建一个Observable，首个参数可以是数组、类数组对象、一个异步对象(比如http响应)、可迭代对象、类Observable对象]
...
- [concatWith](https://rxjs.dev/api/operators/concatWith)
    - 将源observable与参数observable连接在一起 订阅时先运行源observable中的当源observable处理完后处理参数observable，参数可以传多个observable，只有没有complete后才可以进行下一个observable
